...
\begin{itemize}
    ...
    \item \textbf{PNF}: The Physical Network Function receives the TTI messages for radio transmission. It measures the arrival time of these messages and feeds this "Timing Info" back to the VNF, closing the control loop.
\end{itemize}
...
\begin{equation}
    \Delta t = t_{arrival} - t_{slot}
\end{equation}
If $\Delta t > 0$, the packet is late; if $\Delta t < 0$, it is early. The PNF feeds this $\Delta t$ back to the VNF. The scheduler uses this feedback to adjust the start time of the next slot calculation.
