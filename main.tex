...
\author{
  ...
  \IEEEauthorblockA{
    \IEEEauthorrefmark{1}Dept. of Electronic and Computer Engineering, National Taiwan University of Science and Technology, Taiwan\\
...
\color{blue}{
In recent years, the Open Radio Access Network (O-RAN) architecture has become popular.
...
\begin{figure}[htbp]
  \centerline{\includegraphics[width=0.8\columnwidth]{figures/O-RAN-nFAPI.png}}
  \caption{O-RAN nFAPI functional split architecture.}
  \label{fig:O-RAN-nFAPI}
\end{figure}

We define the Virtual Network Function (VNF) to include Layer 2 and Layer 3 operations.
...
\begin{figure}[htbp]
  \centerline{\includegraphics[width=0.8\columnwidth]{figures/Scheduler-to-PNF-latency.png}}
...
The system must often face non-ideal network environments.
In these environments, the network latency is unpredictable.
Figure~\ref{fig:Non-Ideal-Latency} shows the latency jitter in a non-ideal deployment.
Variable latency causes timing issues for the MAC scheduler.
Our research shows that dynamic trigger scheduler adjustment is necessary.
Using outdated timing information reduces network performance and reliability.
This paper proposes an adaptive control method to solve these timing problems.

\begin{figure}[htbp]
  \centerline{\includegraphics[width=0.8\columnwidth]{figures/Non-Ideal-Latency.png}}
