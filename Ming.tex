...
\begin{itemize}
    ...
    \item 運用EURECOM實習期間開發的nFAPI模組和實驗室既有的FAPI實作經驗
...
\begin{itemize}
    ...
    \item 提供標準化的測量框架，支援不同廠商和部署環境的通用性應用
    \item RU支援Metanoia, pegatron, liteon
    \item UE支援root Android
    \item 實現throughput、latency、資源使用率等多維度指標的統整報告
\end{itemize}
...
\begin{itemize}
    ...
    \item 提供可視化的性能分析儀表板和報告生成功能
\end{itemize}
\end{quote}


\textbf{實證性的FAPI vs nFAPI性能數據集}
\begin{itemize}
    \item 全面的比較分析結果
    \item 基於實際測量提供FAPI與nFAPI在各種流量條件下的延遲、吞吐量比較數據
    \item 建立包含系統資源使用效率、可靠性指標的完整性能數據集
...
\begin{itemize}
    ...
    \item 驗證優化效果並提供實作指南，提升整體系統流量處理能力
\end{itemize}

\textbf{工程實踐指南}
\begin{itemize}
    \item 總結OpenAirInterface平台上FAPI/nFAPI實作的關鍵經驗
    \item 建立可重現的實驗流程和測試基準
\end{itemize}
